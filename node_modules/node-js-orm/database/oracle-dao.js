...
const convertWhereDao2Sql = require("./convert-where-dao-2-sql");
...
class OracleDAO {
   ...
   * @param {*} dbconfig
   * @param {*} isDebug
   * @param {*} isGte12c >=12c Là csdl 12C trở lên thì hỗ trợ autoIncrement và paging
   */
  constructor(dbconfig, isDebug, isGte12c = true) {
  ...
  update(updateTable) {
    ...
    let sqlWhere = convertWhereDao2Sql(updateTable.wheres);
  ...
  delete(deleteTable) {
    ...
    let sqlWhere = convertWhereDao2Sql(deleteTable.wheres);
  ...
  select(selectTable) {
    ...
    for (let col of selectTable.cols) {
      sqlColNames += `${sqlColNames ? `, ` : ``}` + col.name;
    }

    if (sqlColNames) sql = `SELECT ${sqlColNames} FROM ${selectTable.name}`;

    let sqlWhere = convertWhereDao2Sql(selectTable.wheres);
  ...
  selectAll(selectTable) {
    ...
    if (selectTable.limitOffset) {
      ...
      // phát hiện oracle 12 thông qua tham số cấu hình bên ngoài đưa vào
      if (this.isGte12c) {
        ...
        // nếu oracle 11 trở xuống thì chạy lệnh này (lưu ý nó sẽ full table scan trước khi lấy trang)
        sql = `SELECT * FROM
                        (
                            SELECT a.*, rownum r__
                            FROM
                            ...
                            WHERE rownum < ((${selectTable.limitOffset.page} * ${selectTable.limitOffset.limit}) + 1 )
                        )
                        WHERE r__ >= (((${selectTable.limitOffset.page}-1) * ${selectTable.limitOffset.limit}) + 1)
                        `;
      }
    }

    return this.getRsts(sql);
  }
  /**
  ...
  /**
   * 
   * Gọi bất kỳ hàm oracle nào có cấu trúc: là một hàm, chứa các tham số input (không chứa tham số output)
   * Kết quả trả về của hàm là dạng stringJSON | text | number (yêu cầu thiết kế bắt buộc trong oracle là như vậy)
   * @param {*} oracleFunctionName 
   */
  executeAnyFunction(oracleFunctionName) {
    // hàm trả về giá trị chuỗi
    let bindVars = {
      v_out: {
        type: this.oracledb.STRING,     // chuỗi đại diện như json (tối đa oracle hỗ trợ là 2000 byte) 
        dir: this.oracledb.BIND_OUT,
      }
    };

    return this.executeFunction(
      `BEGIN :v_out := ${oracleFunctionName}; END;`,
      bindVars
    ).then((result) => {
      return result.v_out;
    });
      ...
      connectRunSql.execute(
        ...
        (err, result) => {
          ...
          if (err) {
            ...
            //Trường hợp createTable sẽ undefined result
          }
        }
      );
    });
  ...
  // Chạy bất kỳ lệnh sql nào theo CRUD để trả kết quả đúng
  runAnySql(sql, params = []) {
    if (!sql || typeof sql !== "string") {
      return Promise.reject("No Sql or type error!");
    }

    let newSql = sql.toLocaleLowerCase();

    if (
      newSql.includes("insert") ||
      newSql.includes("update") ||
      newSql.includes("delete")
    ) {
      return this.runSql(sql, params);
    }

    if (newSql.includes("select")) {
      return this.getRsts(sql, params);
    }

    // yêu cầu hàm gọi phải có dạng tham số vào, nếu ko thì bỏ qua
    // đặt tên hàm trong oracle tránh đặt các tên trùng các từ khóa ở bên trên CRUD
    if (newSql.includes("(") && newSql.includes(")")) {
      return this.executeAnyFunction(sql);
    }

    return Promise.reject(`Sql were not supported! ${sql}`);
