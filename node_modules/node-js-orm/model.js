...
class Model {
  ...
  /**
   * Xóa bảng trong csdl của mô hình này, lệnh nguy hiểm
   */
  drop() {
    return this.db.dropTable(this.tableName);
  }
  ...
  async create(jsonData) {
    ...
    try {
      ...
      // nếu jsonData không có dữ liệu thì không cho insert
      if (!jsonDaoData || typeof jsonDaoData !== "object" || !Object.keys(jsonDaoData).length) {
        throw {
          data: jsonData,
