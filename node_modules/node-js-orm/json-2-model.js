    ...
    if (tableCols && tableCols.length > 0) {
      ...
      for (let col of tableCols) {
        ...
        let fiedlCfg = {
          ...
          // isIndex: col[headerCfg.orm_is_index],
          // indexKeyMulti: col[headerCfg.orm_index_multi],
          ...
          type: col[headerCfg.orm_data_type],
          notNull: parseInt(col[headerCfg.orm_not_null]),
          primaryKey: parseInt(col[headerCfg.orm_primary_key]),
          isUnique: parseInt(col[headerCfg.orm_is_unique]),
          uniqueKeyMulti: col[headerCfg.orm_unique_multi],
          isIndex: parseInt(col[headerCfg.orm_is_index]),
          indexKeyMulti: col[headerCfg.orm_index_multi],
          foreignKey: col[headerCfg.orm_foreign_key],
          autoIncrement: parseInt(col[headerCfg.orm_auto_increment]),
          length: parseInt(col[headerCfg.orm_length]),
          defaultValue: col[headerCfg.orm_default_value],
        };
        jsonTableModel[col[headerCfg.field_name]] = fiedlCfg;
      }
      models[tableName] = jsonTableModel;
    }
  }
...
const jsonText2Model = (jsonTextModel, headerCfg = HEADER_CFG) => {
  ...
  for (let key in jsonTextModel) {
    ...
    if (typeof col === "object") {
      ...
      if (col[headerCfg.orm_is_index] || col["isIndex"])
        fiedlCfg.isIndex = col[headerCfg.orm_is_index] || col["isIndex"];
      if (col[headerCfg.orm_index_multi] || col["indexKeyMulti"])
        fiedlCfg.indexKeyMulti =
          col[headerCfg.orm_index_multi] || col["indexKeyMulti"];
...
module.exports = {
  ...
  modelConfig: {
    ...
    field_name: { type: "STRING", notNull: 1, length: 50 },
    // đặt tên nhận diện để hiển thị trên form nhập liệu hoặc xuất excel
