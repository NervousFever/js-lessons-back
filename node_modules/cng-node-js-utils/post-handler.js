...
var saveToDish = (filename, data, options) => {
  return new Promise((rs, rj) => {
    ...
    // lưu file xuống đĩa kiểm soát việc đóng file và lưu thành công mới trả về
    const read = fs.createReadStream(data)
    const write = fs.createWriteStream(filename, { flags: 'w' })

    function copyit() {
      // console.log('copyit');
      write.on('error', function (err) {
        // console.log('Lỗi lưu file', err);
        rj(err);
      })
    ...
    // sự kiện file có thể đọc được thì gọi nó đọc
...
var jsonProcess = (req, res, next) => {
  ...
  // nếu sử dụng bodyParser để lấy nội dung trước đó thì hàm đợi, lắng nghe sự kiện này sẽ không hoạt động nữa 
  // - dữ liệu sẽ không truyền lên
  // const bodyParser = require('body-parser');
  // app.use(bodyParser.json());
  // app.use(bodyParser.urlencoded({ limit: '10mb', extended: true }));

  if (req.body) {
    req.json_data = req.body;
    next();
    return;
  }
  ...
  req.on('end', () => {
    try {
      // co 2 truong hop
      // neu post json thi '{"":""}'
      // neu post kieu application/x-www-form-urlencoded
      // thi xu ly nhu kieu cua ?request
      req.json_data = JSON.parse(postDataString);
    } catch (err) {
      try {
        req.json_data = JSON.parse('{"' + postDataString.replace(/&/g, '","').replace(/=/g, '":"') + '"}', function (key, value) { return key === "" ? value : decodeURIComponent(value) })
      } catch (e) { }
    }
    next();
  })
...
/**
 * Ứng dụng để xử lý lấy token trên header, json, hoặc request
 * Và lưu trữ file, lấy json khi post lên
 */
module.exports = {
  jsonProcess,
  formProcess,
